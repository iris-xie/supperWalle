...
<body class="gray-bg">
                        ...
                        <?php if ($list): foreach ($list as $v): ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php if ($v['status'] == 1 && $send_test_pri): ?>
                                        <button type="button" class="btn btn-outline btn-info online-send-test" online-id="<?php echo $v['id']; ?>">代码送测</button>
                                    <?php endif; ?>
									<?php if ($v['status'] == 2): ?>
                                        <?php if ($cancel_pri): ?>
                                            <button type="button" class="btn btn-outline btn-warning cancel-send-test" online-id="<?php echo $v['id']; ?>">撤销送测</button>
                                        <?php endif; ?>
                                        <?php if ($submit_wait_prepare_pri): ?>
                                            <button type="button" class="btn btn-outline btn-success submit-prepare" online-id="<?php echo $v['id']; ?>">提交待预发布</button>
                                        <?php endif; ?>
                                    <?php endif; ?>
									<?php if ($v['status'] == -3 && $update_send_test_pri): ?>
                                        <button type="button" class="btn btn-outline btn-info update-send-test" online-id="<?php echo $v['id']; ?>">更新送测</button>
                                    <?php endif; ?>
									<?php if ($edit_pri): ?>
    ...
    <script>
		...
		$(document).ready(function() {
			$(".online-send-test").click(function () {
                var onlineId = $(this).attr("online-id"),
                    postData = {online_id : onlineId};

                parent.layer.confirm("你确认要送测？？？", {
                    icon: 3,
                    btn: ['确定','取消']
                }, function(){
                    $.post("<?php echo site_url('online/submit_send_test');?>", postData, function(res) {
                        parent.layer.close();
                        if (res.status == 1) {
			...
			$(".cancel-send-test").click(function () {
                var postData = {online_id : $(this).attr("online-id")};
                parent.layer.confirm("你确认要撤销送测？？？", {
                    icon: 3,
                    btn: ['确定','取消']
                }, function(){
                    $.post("<?php echo site_url('online/cancel_send_test');?>", postData, function(res) {
                        parent.layer.close();
                        if (res.status == 1) {
                            parent.layer.msg(res.msg, {icon: 1, time: 4000}, function() {
                                window.location.reload();
            ...
            $(".submit-prepare").click(function () {
                var postData = {online_id : $(this).attr("online-id")};
                parent.layer.confirm("你确认要提交待预发布？？？", {
                    icon: 3,
                    btn: ['确定','取消']
                }, function(){
                    $.post("<?php echo site_url('online/submit_wait_prepare');?>", postData, function(res) {
                        parent.layer.close();
                        if (res.status == 1) {
                            parent.layer.msg(res.msg, {icon: 1, time: 4000}, function() {
                                window.location.reload();
			...
			$(".update-send-test").click(function () {
                var postData = {online_id : $(this).attr("online-id")};
                parent.layer.confirm("你确认要更新送测？？？", {
                    icon: 3,
                    btn: ['确定','取消']
                }, function(){
                    $.post("<?php echo site_url('online/submit_update_send_test');?>", postData, function(res) {
                        parent.layer.close();
                        if (res.status == 1) {
                            parent.layer.msg(res.msg, {icon: 1, time: 4000}, function() {
                                window.location.reload();
