...
<form class="form-horizontal m-t submit-apply" method="get" action="<?php echo $url;?>">
    <div class="wrapper wrapper-content animated fadeInRight">
                ...
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        <div class="form-group">
                            ...
                            <div class="col-sm-8">
                                <b><?php echo $channel_list[$cid]['name'];?></b>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-3 control-label">上线单标题：</label>
                            <div class="col-sm-8">
                                <input type="text" name="title" class="form-control" placeholder="上线单标题" value="<?php echo $info['title'];?>" />
                            </div>
                        </div>
                        <div class="form-group" style="display: none;">
                            ...
                            <div class="col-sm-8 input-append">
                                ...
                                <input type="hidden" name="old_revision" value="<?php echo $info['start']; ?>" />
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-8 input-group">
                                <select name="new_revision" class="form-control">
                                    <?php foreach($data['server_log'] as $d): ?>
                                        <option value="<?php echo $d['revision']; ?>" <?php echo $d['revision'] == $info['end'] ? 'selected' : ''; ?> author="<?php echo $d['author']; ?>"><?php echo $d['revision'].'#'.$d['author'].'#'.date('Y-m-d H:i:s', $d['date']).'#'.$d['msg']; ?></option>
                                    <?php endforeach; ?>
                                </select>
								<span class="input-group-btn">
                                    <button type="button" class="btn btn-info show-self-revision">仅自己</button>
                                </span>
                            </div>
                        </div>
						<div class="form-group">
							...
							<div class="col-sm-8">
								<textarea name="demand" class="form-control" placeholder="产品需求" rows="5"><?php echo $info['demand'];?></textarea>
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-3 control-label">测试注意点：</label>
							<div class="col-sm-8">
								<textarea name="test_note" class="form-control" placeholder="测试注意点" rows="5"><?php echo $info['test_note'];?></textarea>
							</div>
						</div>
                    </div>
                </div>
		...
		<div class="row">
			<?php if ($info['status'] == -3 || $info['status'] == 2): ?>
				<div class="col-sm-12">
					<div class="ibox-title">
						<h5>已选择更新文件列表</h5>
					</div>
							...
							<table class="table table-striped">
								...
								<tbody class="change-list-tbody">
									<?php foreach ($files as $f): ?>
										<tr>
											<td><input type="hidden" name="item[]" value="added"><?php echo $f['item'];?></td>
											<td><input type="hidden" name="kind[]" value="<?php echo $f['kind'];?>"><?php echo $f['kind'];?></td>
											<td><input type="hidden" name="name[]" value="<?php echo $f['name'];?>"><?php echo $f['name'];?></td>
											<td><input type="hidden" name="revision[]" value="<?php echo $f['name'];?>"><input type='checkbox' name='check[]' class='i-checks' value='<?php echo $f['name'];?>' checked /></td>
										</tr>
									<?php endforeach; ?>
								</tbody>
							</table>
						...
						<table class="table table-striped change-list">
							<thead>
								<tr>
									...
									<th>文件类型</th>
									<th>文件名</th>
									<th><input type='checkbox' name='allCheck' value='1' /></th>
								</tr>
							</thead>
							<tbody class="change-list-tbody">
								<tr><td colspan="3" align="center">正在加载中...</td></tr>
							</tbody>
						</table>
					</div>
				</div>
			...
			<div class="col-sm-12 ibox-content text-center">
				...
				<input type="hidden" name="online_id" value="<?php echo $info['id']; ?>" />
				<input type="hidden" name="type" value="edit" />
				<input class="btn btn-large btn-block btn-primary" type="submit" value="编辑上线单">
			</div>
		</div>
    </div>
    </form>
    ...
    <script>
		...
		$(document).ready(function() {
            ...
            var $files = new Array();
            <?php
                $files_arr = json_decode($info['files'], TRUE);
                foreach ($files_arr as $f):
            ?>
            $files.push("<?php echo $f['name'];?>");
            <?php endforeach; ?>
            ...
            var diff_func = function() {
                ...
                $.post("<?php echo site_url('online/create');?>", postData, function(res) {
                    if (res.status == 1) {
                        ...
                        if (data.length > 0) {
                            ...
                            for (var d in data) {
                                ...
                                var isChecked = $.inArray(data[d].name, $files) > -1 ? "checked" : "";
                                ...
                                html += "<td><input type='checkbox' name='check[]' class='i-checks' value='"+name+"' "+isChecked+"><input type='hidden' name='revision[]' value='"+old_revision+"'></td>";
                                html += "</tr>";
                            }
                        }
